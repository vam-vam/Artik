...
# Parts of code adapted from https://github.com/chrisb2/pyb_ina219 (MIT License)
...
if __name__ == "__main__":
    logging.basicConfig(
        level=logging.ERROR,
        format='%(asctime)s - %(levelname)s - INA219 %(message)s',
    )
    ina = INA219(shunt_ohms=0.1, max_expected_amps=3.0, address=0x44, log_level=logging.WARNING)
    ina.configure(voltage_range=ina.RANGE_32V, gain=ina.GAIN_AUTO, bus_adc=ina.ADC_128SAMP, shunt_adc=ina.ADC_128SAMP)

    while 1:
        try:
            print("Bus Voltage/vstup    : %.3f V" % ina.voltage())
            print("Bus Current/proud    : %.3f mA" % ina.current())
            print("Supply Voltage       : %.3f V" % ina.supply_voltage())
            print("Shunt voltage/bocnik : %.3f mV" % ina.shunt_voltage())
            print("Power/vykon          : %.3f mW" % ina.power())
            print("")
